...
| `.tbl-vazio` | modificador da `.tbl-band`: a ressalva centrada na moldura, quando o cartão não tem o que desenhar | aba Distribuição em área sem referência |
...
| `peer group`                  | Conjunto de cooperados da mesma área de atuação (= especialidade) — base de toda comparação justa. **Sub-perfil (cirurgia, executa, área secundária, carteira jovem/climatério) é IDENTIDADE visível — etiqueta na linha —, nunca subdivisão de régua nem recorte** (o filtro por sub-perfil saiu da tela de Área em set/2026: ver METODOLOGIA §5.8)                                                                  |
